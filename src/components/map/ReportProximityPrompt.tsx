...
const REPORT_META: Record<string, { label: string; color: string; bg: string; border: string; icon: string }> = {
  POLICE:          { label: 'Policja',            color: 'text-blue-400',   bg: 'bg-blue-500/20',   border: 'border-blue-500/40',   icon: 'M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z' },
  ...
  SPEED_TRAP:      { label: 'Kontrola prędkości', color: 'text-yellow-400', bg: 'bg-yellow-500/20', border: 'border-yellow-500/40', icon: 'M15 10.5a3 3 0 11-6 0 3 3 0 016 0z M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z' },
      ...
      <div className={`rounded-3xl border ${meta.border} bg-card-bg/95 backdrop-blur-xl shadow-2xl overflow-hidden`}>
        ...
        <div className="flex items-center gap-4 px-5 pt-5 pb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {isOwner && (
              <p className="text-xs text-muted/50 mt-0.5">Twój raport</p>
            )}
          </div>
        ...
        <p className="px-5 pb-4 text-sm text-muted">
          {isOwner
            ? 'Czy Twój raport jest nadal aktualny?'
        ...
        <div className="flex gap-3 px-5 pb-5">
          {isOwner ? (
            ...
            <button
              ...
              disabled={isVoting}
              className="flex-1 flex items-center justify-center gap-2.5 rounded-2xl bg-green-600 hover:bg-green-500 active:bg-green-700 text-white font-bold text-base py-4 transition disabled:opacity-50 shadow-lg active:scale-[0.98]"
            >
